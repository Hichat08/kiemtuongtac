...
import { getWithdrawalBankAppearance } from "@/lib/withdraw-bank-accounts";
import {
...
const getBankBadge = (request: WithdrawalRequest) =>
...
export default function WithdrawalSuccessPage() {
  ...
  if (request.status !== "approved") {
    return (
      <Navigate
        to={`/wallet/withdraw/pending/${request.id}`}
        replace
      />
    );
  }
  ...
  const bankBadge = getBankBadge(request);
      ...
      <main className="mobile-page-shell pb-36 pt-8">
        <section className="flex flex-col items-center text-center">
          ...
          <h1 className="font-auth-headline text-[2.1rem] font-extrabold tracking-[-0.05em] text-[#2f2441] dark:text-white">
            {internalTransfer ? "Chuyển tiền nội bộ thành công!" : "Rút tiền thành công!"}
          </h1>
          <p className="mt-3 max-w-sm text-sm leading-7 text-[#7e7691] dark:text-[#c8b5e8]">
            {internalTransfer
              ? "Hệ thống đã chuyển tiền ngay tới tài khoản nội bộ người nhận."
        ...
        <section className="mt-10 rounded-[1.7rem] bg-white/88 p-6 shadow-[0_24px_60px_-40px_rgba(123,25,216,0.26)] backdrop-blur-2xl dark:bg-white/8">
          <div className="text-center">
            <p className="text-sm font-medium text-[#8d84a1] dark:text-[#bdaaD6]">
              {internalTransfer ? "Số tiền đã chuyển" : "Tổng số tiền nhận được"}
            </p>
          ...
          <div className="mt-8 space-y-5">
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <span className="font-auth-headline text-base font-bold text-[#7b19d8] dark:text-[#ff84d1]">
                Miễn phí
              </span>
            </div>
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2">
                <div
                  className={`flex size-7 items-center justify-center rounded-lg text-[10px] font-auth-headline font-bold ${bankBadge.logoClassName}`}
                >
                  {bankBadge.logoLabel}
                </div>
