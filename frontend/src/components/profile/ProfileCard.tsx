    ...
    <Card className="overflow-hidden p-0 h-52 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500">
      <CardContent className="mt-20 pb-8 flex flex-col sm:flex-row items-center sm:items-end gap-6">
        ...
        <div className="text-center sm:text-left flex-1">
          ...
          <div className="mt-2 flex flex-wrap items-center justify-center gap-2 text-xs font-semibold text-white/85 sm:justify-start">
            <span>@{user.username}</span>
            {user.accountId ? (
              <span className="rounded-full bg-white/14 px-3 py-1 font-mono tracking-[0.2em] text-white">
                ID {user.accountId}
              </span>
            ) : null}
          </div>
